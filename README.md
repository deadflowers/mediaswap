...
### Desktop non-app
Mediaswap is a robust wrapper designed to bring the power of **FFmpeg** and **ImageMagick** to your Ubuntu/GNOME desktop context menus. **Mediaswap** desktop GUI has no central app body, there are no new heavy programs to open or learn. Essentially a **Zenity** powered non-app that attaches itself to your shell, integrating seamlessly into your environment, and organically into your workflow. When functions are requested menus and dialogues are instantiated and close when task is complete.

Not residing in memory and being only around 500 lines, it's like it's not even there yet it's always one click away from converting 100s of movies to the latest video codec, web friendly format, extracting bit perfect audio streams for each, converting your web graphics, adding shadows, making modern GIF animations of stills using WebP. Slash your server or AWS bill by shrinking your media file sizes, using mediaswap to help with bulk conversions be it sound files, videos, static or animated images.
...
The objective is to give console users a simpler, faster way to call on the most common functions. The power of FFMpeg extended to all experience levels including novice user who should be able to rip bit perfect audio, migrate their videos to av1, while knowing almost nothing about ffmpeg. The commands should too simple to confuse, too sensible to forget, too short to typo, and the help docs should fit on one page that can be absorbed in under a minute. 
